...
def build_invoice_units(profile: FileProfile) -> list[InvoiceUnit]:
    ...
    if profile.file_type == FileType.PDF and profile.ocr_required:
        ...
        unit_type = "text_file"
    elif profile.file_type == FileType.XML:
        unit_type = "xml"
...
def merge_invoice_units(profile: FileProfile, units: list[InvoiceUnit]) -> InvoiceUnit:
    page_range = [page for unit in units for page in unit.page_range]
